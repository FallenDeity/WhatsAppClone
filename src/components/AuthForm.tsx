			...
			<div className="relative rounded-lg bg-white px-6 py-6 shadow-lg dark:bg-gray-900 sm:px-10">
				...
				<form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
					{variant === FormVariants.REGISTER && (
						<Input disabled={loading} register={register} errors={errors} required id="name" label="Name" />
					)}
					<Input
						disabled={loading}
						register={register}
				...
				<div className="mt-6 flex justify-center gap-2 px-2 text-sm text-gray-500">
					<div>{variant === FormVariants.LOGIN ? "New to whatsapp?" : "Already have an account?"}</div>
